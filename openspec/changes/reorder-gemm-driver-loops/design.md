...
当前 GEMM 的 driver 采用三层阻塞循环，但循环层级中 `col1` 位于 `row1` 外层，导致同一个 `(row1, k0)` 的 `MR x Q` A 微块在处理多个 B 列块时被重复打包。这个问题不会改变计算结果，但会显著放大 packing 开销，并在较大矩阵下压低整体 GFLOPS。
...
**Goals:**
- 让同一 `(row1, k0)` 的 `MR x Q` A 微块只打包一次
- 在同一 `(col0, k0)` 的 `R` 范围内复用 packed A 给多个 `col1` / `NR` B 微块
- 先打包当前 `(col0, k0)` 范围内所有 `Q x NR` B 微块，使 `sb` 形成逻辑上的 `Q x R` packed B panel
...
**Decision**: 对每个 `(col0, k0)`，先遍历并打包当前 `R` 范围内所有 `col1` 对应的 `Q x NR` B 微块；随后遍历 `row0/row1`，对每个 `MR x Q` A 微块只执行一次 `pack_A`，并在内层遍历当前 `R` 范围内所有 `col1` 调用 kernel。

**Rationale**: 这样可以把 A 的打包结果从“每个 B 列块都重复计算一次”变成“每个 `(row1, k0)` 只计算一次，并复用给多个 `Q x NR` B 微块”。对当前工作负载而言，A 的重复打包是主要瓶颈，优先消除它能获得最大的收益。
...
**Decision**: 不调整 packing buffer 的内存布局和偏移策略，仅改变它们的使用顺序。`sb` 继续按 `k_rem * (col1 - col0)` 存放各个 `Q x NR` B 微块，从而在逻辑上构成当前 `(col0, k0)` 的 `Q x R` packed B panel。
...
- [Risk] 当前 `(col0, k0)` 范围内的 B 微块会被提前完成打包，可能增加 `sb` 的驻留时间 → 通过保持 `R` 范围内局部性和不改变缓冲区大小来缓解
