...
- CPU: Apple Mac (AVX2 not supported, 仅 generic 路径)
- 编译器: gcc -O2 -Wall
- 精度: double
- 阻塞参数: P=128, Q=128, R=4096, MR=4, NR=4
...
Single-thread, generic (no SIMD) 路径：

| 矩阵大小 | GFLOPS | 迭代次数 | 耗时 (s) |
|----------|--------|----------|----------|
| 64×64    | 3.64   | 20       | 0.003    |
| 128×128  | 2.76   | 20       | 0.030    |
| 256×256  | 3.51   | 10       | 0.096    |
| 512×512  | 3.55   | 5        | 0.378    |
| 1024×1024 | 3.55  | 5        | 3.026    |

CPU 理论峰值约为 20-40 GFLOPS（取决于具体型号），当前 generic 路径约 3.5 GFLOPS，**达到理论的 10-18%**。瓶颈在于纯 C 标量代码无 SIMD，且打包开销占比显著。

### 5.4 与 OpenBLAS 对比

测试环境：Apple M5（ARM64，无 AVX2），gcc -O2，OpenBLAS 0.3.29（ARM64 NEON 优化路径）。

**Single-thread：**

| 矩阵大小 | MyBLAS (GFLOPS) | OpenBLAS (GFLOPS) | 差距 |
|----------|-----------------|-------------------|------|
| 64×64    | 1.79            | 17.74             | 9.9x |
| 128×128  | 1.89            | 42.24             | 22.3x |
| 256×256  | 3.31            | 129.65            | 39.2x |
| 512×512  | 3.52            | 195.77            | 55.7x |
| 1024×1024 | 3.54           | 248.11            | 70.2x |
| 2048×2048 | 3.41           | 282.18            | 82.8x |

**4 threads：**

| 矩阵大小 | MyBLAS (GFLOPS) | OpenBLAS (GFLOPS) | 差距 |
|----------|-----------------|-------------------|------|
| 64×64    | 7.38            | 45.20             | 6.1x |
| 128×128  | 10.30           | 50.11             | 4.9x |
| 256×256  | 11.56           | 150.47            | 13.0x |
| 512×512  | 11.84           | 160.70            | 13.6x |
| 1024×1024 | 12.75          | 239.43            | 18.8x |
| 2048×2048 | 13.21          | 284.13            | 21.5x |

**分析：**

- **小矩阵 (64×64)**：差距最小（6-10x），此时打包开销占比大，计算密度低，SIMD 优势不能充分发挥。
- **大矩阵 (2048×2048)**：差距最大（22-83x），OpenBLAS 的 ARM64 NEON 内核达到 **282 GFLOPS**（M5 理论峰值约 ~350 GFLOPS，效率 ~80%），而 MyBLAS 纯 C 标量受限于无 SIMD 瓶颈。
- **MyBLAS 多线程扩展性**：1→4 线程仅获得 3.0-4.1x 加速，接近线性；但 OpenBLAS 4 线程在 64/128 小矩阵上反而因线程管理开销而性能下降。
- **核心差距**：MyBLAS 当前使用 generic C 三重循环微内核（每次迭代 2 次 load + 2 次乘加 = 4 FLOP，无向量化），而 OpenBLAS 使用 NEON 向量化内核（每次迭代 2 条 `fmla` 指令处理 8 个 double = 16 FLOP），计算密度差 4 倍；加上 cache/TLB 参数的深度调优、prefetch 等，累积出 20-80x 的差距。
